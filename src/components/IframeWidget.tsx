...
import { snapshotCssVars } from '../utils/widgetShare';
 ...
 * Extracts <style>, <script>, and markup separately to avoid Babel issues.
 */
function buildHtmlDocument(widgetCode: string): string {
  ...
  // Extract <style>, <script>, and markup separately
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: var(--font-sans); background: transparent; padding: 8px; overflow-x: hidden; }
  ${styles}
</style>
</head>
<body>
...
export function IframeWidget({ code, onSendPrompt, onShare }: IframeWidgetProps) {
  ...
  const handleShare = useCallback(async () => {
    if (!onShare) return;
    const cssVars = snapshotCssVars();
    const url = await onShare({ widgetCode: code, cssVars });
    if (url) {
    ...
    <div className="my-3 relative group">
      ...
      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {onShare && (
          <button
            onClick={handleShare}
            className="p-1.5 text-gray-400 hover:text-gray-600 bg-white/80 hover:bg-white rounded shadow-sm"
